...
//                painted like a compose (compose.mjs generateLandscape).
 ...
 * 'opus'); the owner's saved copy of the piece gets `channel`. The caller paints.
 */
export async function jamChannel({ db, piece, owner = null, ask = askOpus }) {
  const E = await loadPieceEngine();
  const draft = E.pieceToLandscape(piece, baseLandscape(db, piece.base, owner, E.LANDSCAPES));
  const brief = piece.feel?.brief?.trim() ?? '';
  const prompt = [brief, `(made in jam from "${piece.name}")`].filter(Boolean).join('\n\n');
  const first = buildChannelPrompt({ piece, draft, brief, E });
  const res = await generateLandscape({ db, first, prompt, owner, ask, label: 'jam-channel' });
